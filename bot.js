...
var token = "NjU1MjQwODIwNTk2OTMyNjA4.Xidu0Q.YW2Ib89AN1t6YFxA6FEnJtBl0Mc";//auth.debugToken;
...
bot.on('message', (message) => {
    //If from andrezza
    if(message.author.id != "272471724065554433") {
        var cont = message.content.toLowerCase();
        console.log("Arzin said: " + cont)
        switch(cont) {
            case 'fuck you':
            case 'fuk u':
            case 'fuck u':
            ...
            case 'fack me':
            case 'fuck meh':
                message.channel.send('você gostaria');
                break;
            default:
                break;
        }
    ...
    var invoker = message.content.substring(0, 1);

    if(invoker == '!') {
        message.content = message.content.substring(1, message.content.length);
        console.log(invoker + "  " + message.content);
        //If from self
